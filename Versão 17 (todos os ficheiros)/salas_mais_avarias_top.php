...
$op2 = ($op === 't') ? 'Todos os anos letivos' : 'Ano letivo: ' . htmlspecialchars($ano_max, ENT_QUOTES, 'UTF-8');

// ── 6. Query principal ───────────────────────────────────────────────────────
if ($op === 't') {
    $stmt = mysqli_prepare($db,
        "SELECT ar.id_sala, s.nome, MAX(ar.ano_letivo) AS ano_letivo, COUNT(*) AS c
         FROM avarias_reparacoes ar
         INNER JOIN salas s ON s.id = ar.id_sala
         WHERE ar.id_escola = ?
         GROUP BY ar.id_sala, s.nome
         ORDER BY c DESC, ar.id_sala
         LIMIT 10"
    );
    mysqli_stmt_bind_param($stmt, 'i', $idescola);
} else {
    $stmt = mysqli_prepare($db,
        "SELECT ar.id_sala, s.nome, ar.ano_letivo, COUNT(*) AS c
         FROM avarias_reparacoes ar
         INNER JOIN salas s ON s.id = ar.id_sala
         WHERE ar.ano_letivo = ? AND ar.id_escola = ?
         GROUP BY ar.id_sala, s.nome, ar.ano_letivo
         ORDER BY c DESC, ar.id_sala
         LIMIT 10"
    );
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Salas com mais avarias (Top 10)</li>
                  </ol>
               </nav>
...
<?php include("msg_bemvindo.php"); ?>
                  ...
                  <div style="display:flex;gap:10px;margin-bottom:18px;flex-wrap:wrap;">
                     <a href="<?php echo SVRURL ?>salas_mais_avarias_top.php?op=t&&ies=<?php echo base64_encode($idescola); ?>"
                     ...
                     <a href="<?php echo SVRURL ?>salas_mais_avarias_top.php?op=al&&ies=<?php echo base64_encode($idescola); ?>"
...
?>
            <tr>
                ...
                <td data-label="Sala" style="font-weight:700;">
                ...
                <td data-label="Nº avarias">
                    <a class="gei-link" title="Ver avarias/reparações da sala"
                       href="<?php echo SVRURL ?>num_avarias_sala.php?x=<?php echo base64_encode(2); ?>&&si=<?php echo base64_encode($row['id_sala']); ?>&&ies=<?php echo base64_encode($idescola); ?>">
