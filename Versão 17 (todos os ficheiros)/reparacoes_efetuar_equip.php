...
// Prepared statements — sem interpolação directa
$stmt10 = $db->prepare("SELECT nomeequi FROM equipamento WHERE id=?");
$stmt10->bind_param("i", $eq);
$stmt10->execute();
...
$stmt9 = $db->prepare("SELECT nome FROM salas WHERE id=?");
$stmt9->bind_param("i", $said);
$stmt9->execute();
$rows9 = $stmt9->get_result()->fetch_row();
$stmt9->close();
$ns    = $rows9[0] ?? '';
$num_ns = $ns !== '' ? 1 : 0;
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Reparações a efetuar — <?php echo htmlspecialchars($noeq, ENT_QUOTES, 'UTF-8'); ?></li>
                  </ol>
               </nav>
...
<?php include("msg_bemvindo.php"); ?>
               ...
               <!-- Card instituição + sala + equipamento -->
               <div style="display:flex; align-items:center; flex-wrap:wrap; gap:16px; margin:14px 0 10px; padding:12px 16px; background:#f4f6fb; border:1px solid #e3e8f4; border-radius:10px;">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#4b6cb7" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
                  ...
                  <div style="display:flex; flex-direction:column;">
                     ...
                     <span style="font-size:.95rem; font-weight:700; color:#182848;"><?php echo htmlspecialchars($noeq, ENT_QUOTES, 'UTF-8'); ?></span>
                  </div>
               </div>
...
$stmt_main = $db->prepare("SELECT * FROM avarias_reparacoes WHERE datareparacao IS NULL AND id_equi=? AND id_escola=? ORDER BY dataavaria DESC LIMIT ?,?");
$stmt_main->bind_param("iiii", $eq, $idescola, $paginationStart, $limit);
$stmt_main->execute();
$result = $stmt_main->get_result();

$stmt_cnt = $db->prepare("SELECT count(*) FROM avarias_reparacoes WHERE datareparacao IS NULL AND id_equi=? AND id_escola=?");
$stmt_cnt->bind_param("ii", $eq, $idescola);
$stmt_cnt->execute();
$rowsCnt     = $stmt_cnt->get_result()->fetch_row();
$totallinhas = (int)($rowsCnt[0] ?? 0);
...
<div class="d-flex flex-row-reverse bd-highlight mb-3">
    <form action="<?php echo SVRURL ?>reparacoes_efetuar_equip.php?ieq=<?php echo base64_encode($eq); ?>&&sai=<?php echo base64_encode($said); ?>&&ies=<?php echo base64_encode($idescola); ?>" method="post" name="form1">
...
<div class="gei-table-wrap">
    <div class="gei-table-section-header">
        ...
        Reparações a efetuar — <?php echo htmlspecialchars($noeq, ENT_QUOTES, 'UTF-8'); ?>
    </div>
...
<?php while ($row = mysqli_fetch_array($result)):
    ...
    $stmt2   = $db->prepare("SELECT nome FROM utilizadores WHERE email=?");
    $stmt2->bind_param("s", $em);
    $stmt2->execute();
    $rows2   = $stmt2->get_result()->fetch_row();
    $stmt2->close();
    $nomeautor = $rows2[0] ?? '—';
?>
            <tr>
...
<nav aria-label="Paginação">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page <= 1) { echo '#'; } else {
                   echo '?ieq='.base64_encode($eq).'&&sai='.base64_encode($said).'&&ies='.base64_encode($idescola).'&&page='.$prev;
               } ?>">&lt;&lt;</a>
        </li>
        ...
        <li class="page-item <?php if ($page == $i) echo 'active'; ?>">
            <a style="color:black;" class="page-link"
               href="reparacoes_efetuar_equip.php?ieq=<?php echo base64_encode($eq); ?>&&sai=<?php echo base64_encode($said); ?>&&ies=<?php echo base64_encode($idescola); ?>&&page=<?php echo $i; ?>">
               <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
        ...
        <li class="page-item <?php if ($page >= $totoalPages) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page >= $totoalPages) { echo '#'; } else {
                   echo '?ieq='.base64_encode($eq).'&&sai='.base64_encode($said).'&&ies='.base64_encode($idescola).'&&page='.$next;
               } ?>">&gt;&gt;</a>
        </li>

        <li class="page-item">
            <?php echo str_repeat("&nbsp;", 5) . "TOTAL: " . $totallinhas; ?>
        </li>
...
<a href="<?php echo SVRURL ?>ver_equipamentos_sala.php?x=<?php echo base64_encode(1) ?>&&si=<?php echo base64_encode($said) ?>&&ies=<?php echo base64_encode($idescola) ?>">
