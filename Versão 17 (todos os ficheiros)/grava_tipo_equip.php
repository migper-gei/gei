...
// [CORRIGIDO] Verificar sessão activa antes de qualquer operação de escrita.
// Antes não havia qualquer verificação — qualquer pedido POST era aceite.
if (empty($_SESSION['user_id'])) {
    header('Location: ' . SVRURL . 'login');
    exit;
}

// [CORRIGIDO] Verificar permissão: só administradores (tipo == 1) podem inserir.
...
// Antes este ficheiro não validava nenhum token — um atacante podia forjar
// qualquer pedido POST a partir de outro site.
if (
...
<?php
    ...
    $stmt_chk = $db->prepare("SELECT COUNT(*) FROM tipos_equipamento WHERE nome = ?");
...
swal({
    title: 'Já existe!',
    text: 'Este tipo de equipamento já está registado!',
    icon: 'warning',
})
.then(function() {
...
<?php
        ...
        // Inserir
        $stmt_ins = $db->prepare("INSERT INTO tipos_equipamento (nome) VALUES (?)");
