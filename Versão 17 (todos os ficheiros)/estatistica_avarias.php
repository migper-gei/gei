...
// ── 1. Validar presença de ies ───────────────────────────────────────────────
if ( empty($_GET["ies"]) || !isset($_GET["ies"]) ) {
...
$stmt = mysqli_prepare($db,
    "SELECT ar.ano_letivo, ar.periodo, COUNT(*) AS cp,
...
mysqli_stmt_bind_param($stmt, 'iii', $idescola, $paginationStart, $limit);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

// ── 7. Total para paginação ──────────────────────────────────────────────────
$stmt_total = mysqli_prepare($db,
    "SELECT COUNT(DISTINCT CONCAT(ano_letivo, periodo)) AS total
     FROM avarias_reparacoes
     WHERE id_escola = ?"
);
mysqli_stmt_bind_param($stmt_total, 'i', $idescola);
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Estatística de avarias</li>
                  </ol>
               </nav>
...
<div class="gei-table-wrap">
    <div class="gei-table-section-header">
        ...
        Estatística de avarias por ano / período de tempo
    </div>
...
<?php while ($row = mysqli_fetch_array($result)): ?>
            <tr>
                ...
                <td data-label="Nº avarias">
                    <span class="gei-badge"><?php echo (int) $row['cp']; ?></span>
                </td>
            </tr>
<?php endwhile; ?>
        </tbody>
...
<nav aria-label="Paginação">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page <= 1) { echo '#'; } else {
                   echo SVRURL . 'estatistica_avarias.php?ies=' . base64_encode($idescola)
                      . '&&page=' . $prev;
               } ?>">&lt;&lt;</a>
        </li>
        ...
        <li class="page-item <?php if ($page == $i) echo 'active'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php echo SVRURL; ?>estatistica_avarias.php?ies=<?php echo base64_encode($idescola); ?>&&page=<?php echo $i; ?>">
               <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
