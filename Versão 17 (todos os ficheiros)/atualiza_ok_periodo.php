            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Períodos >> Atualizar</li>
                  </ol>
               </nav>
               <div class="titlepage">
...

{
    ...
    if (
        ...
        swal({
            ...
            text: 'Token CSRF inválido. Por favor, tente novamente.',
            icon: 'error',
        })
        .then(function() {
            window.location = "<?php echo SVRURL ?>atualizaper/<?php echo base64_encode($ip); ?>";
        });
        </script>
        <?php
        exit;
...
$_al  = $_POST["anoletivo"] ?? '';
$_np  = (int)($_POST["periodo"] ?? 0);
$_di  = $_POST["datai"] ?? '';
$_df  = $_POST["dataf"] ?? '';
$stmt = $db->prepare("UPDATE periodos SET ano_lectivo=?, num_periodo=?, data_inicio=STR_TO_DATE(?,'%Y-%m-%d'), data_fim=STR_TO_DATE(?,'%Y-%m-%d') WHERE id=?");
$stmt->bind_param("sissi", $_al, $_np, $_di, $_df, $ip);
$stmt->execute();
$stmt->close();
