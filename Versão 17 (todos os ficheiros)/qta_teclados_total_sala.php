            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Quantidade de teclados por sala</li>
                  </ol>
               </nav>
...
@media(max-width:768px){
    .gei-table-wrap{border-radius:8px;margin-bottom:14px;margin-left:0;margin-right:0;}
    .gei-table-section-header{font-size:.80rem;padding:10px 14px;}
    .gei-table thead{display:none;}
    .gei-table tbody tr{display:flex;flex-direction:column;border:none;border-bottom:1px solid #eef1f8;padding:10px 14px;background:#fff !important;}
    .gei-table tbody tr:last-child{border-bottom:none;}
    .gei-table td{display:flex;justify-content:space-between;align-items:center;padding:6px 0;border:none;font-size:.83rem;}
    .gei-table td::before{content:attr(data-label);font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:#7b88a0;flex-shrink:0;margin-right:10px;}
    .gei-badge{font-size:.80rem;padding:3px 12px;}
    .gei-total-badge{font-size:.70rem;padding:2px 7px;}
}
</style>
</div>

<?php
// NOTA: adicionar coluna à BD antes de usar este ficheiro:
//   ALTER TABLE equipamento ADD COLUMN tecladointerface varchar(50) DEFAULT NULL AFTER teclado;

// Buscar salas da escola que têm equipamento com tecladointerface preenchido
$sql0 = "
    SELECT s.id, s.nome
    FROM salas s
    ...
    AND EXISTS (
        ...
        AND e.tecladointerface IS NOT NULL AND e.tecladointerface <> ''
    )
    ORDER BY s.nome
";
...
<?php
// CORRECÇÃO 1: removidos if(true), bloco de cabeçalho duplicado e query desnecessária de validação por escola
// CORRECÇÃO 2: </tbody></table></div><br> movidos para DENTRO do while — cada sala fecha a sua tabela
// CORRECÇÃO 3: mysqli_close() movido para FORA do loop
// NOVO: total calculado antes de renderizar o cabeçalho da coluna

while ($row0 = mysqli_fetch_array($result0)) {
    $idsa = $row0['id'];
    $nos  = $row0['nome'];

    // Buscar dados e calcular total ANTES de renderizar a tabela
    $sql01 = "
        SELECT tecladointerface, COUNT(id) AS qta
        FROM equipamento
        WHERE id_sala = $idsa
        AND tecladointerface IS NOT NULL AND tecladointerface <> ''
        GROUP BY tecladointerface
        ORDER BY tecladointerface ASC
    ";
...
<div class="gei-table-wrap">
    ...
    <table class="gei-table">
        ...
        <?php foreach ($rows01 as $row) { ?>
            <tr>
                <td data-label="Interface"><?php echo htmlspecialchars($row['tecladointerface'], ENT_QUOTES, 'UTF-8'); ?></td>
...
<?php mysqli_close($db); // CORRECÇÃO 3: fora do loop ?>
