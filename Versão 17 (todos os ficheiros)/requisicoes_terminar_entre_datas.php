...
<script>
function confirmarEntregaDatas(rid, d1, d2, esc) {
    ...
    swal({
        ...
        if (isConfirm) {
            window.location.href = '<?php echo SVRURL ?>entregar_requisicao_datas.php?ir=' + rid + '&d1=' + d1 + '&d2=' + d2 + '&ies=' + esc;
        } else {
            swal("Cancelado.");
        }
    });
...
if ($x === 0) {
    ...
    $d1 = $_GET['d1'] ?? '';
    $d2 = $_GET['d2'] ?? '';
} else { // x === 2 (paginação / num_linhas)
    $d1 = !empty($_GET['d1']) ? base64_decode($_GET['d1']) : '';
    $d2 = !empty($_GET['d2']) ? base64_decode($_GET['d2']) : '';
}
...
$stmt = mysqli_prepare($db,
    "SELECT r.id AS rid, r.*, s.nome, s.id AS sid, u.nome AS nu
     ...
     WHERE r.datautil BETWEEN STR_TO_DATE(?, '%Y-%m-%d') AND STR_TO_DATE(?, '%Y-%m-%d')
       AND r.dataentrega IS NULL
       AND s.id_escola = ?
     ORDER BY r.datautil
     LIMIT ?, ?"
);
mysqli_stmt_bind_param($stmt, 'ssiii', $d1, $d2, $esc, $paginationStart, $limit);
...
mysqli_stmt_bind_param($stmt_total, 'ssi', $d1, $d2, $esc);
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Requisições a terminar entre datas</li>
                  </ol>
               </nav>
                  ...
                  <div style="display:flex;align-items:center;flex-wrap:wrap;gap:16px;margin:14px 0 10px;padding:12px 16px;background:#f4f6fb;border:1px solid #e3e8f4;border-radius:10px;">
                     ...
                     <div style="display:flex;flex-direction:column;">
                        ...
                        <span style="font-size:.95rem;font-weight:700;color:#182848;">
                            <?php echo date('d/m/Y', strtotime($d1)); ?> &ndash; <?php echo date('d/m/Y', strtotime($d2)); ?>
                        </span>
                     </div>
...
<div class="d-flex flex-row-reverse bd-highlight mb-3">
    <form action="" method="post">
        <input type="hidden" name="datai1" value="<?php echo htmlspecialchars($d1, ENT_QUOTES); ?>">
        <input type="hidden" name="dataf1" value="<?php echo htmlspecialchars($d2, ENT_QUOTES); ?>">
...
<div style="padding:30px 16px;text-align:center;color:#7b88a0;font-size:.9rem;background:#f4f6fb;border:1px solid #e3e8f4;border-radius:10px;">
    ...
    Não existem requisições a terminar neste intervalo de datas.
</div>
...
<div class="gei-table-wrap">
    <div class="gei-table-section-header">
        ...
        Requisições a terminar entre datas
    </div>
            ...
            <tr>
                ...
                <td data-label="Ação" style="vertical-align:middle;">
                    <a class="gei-btn-entregar"
                       onclick="confirmarEntregaDatas(<?php echo $rid; ?>,'<?php echo $d1; ?>','<?php echo $d2; ?>',<?php echo $esc; ?>);"
                       href="<?php echo SVRURL ?>entregar_requisicao_datas.php"
...
<nav aria-label="Paginação">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page <= 1) { echo '#'; } else {
                   echo SVRURL . 'requisicoes_terminar_entre_datas.php?x=' . base64_encode(2)
                      . '&&d1='  . base64_encode($d1)
                      . '&&d2='  . base64_encode($d2)
        ...
        <li class="page-item <?php if ($page == $i) echo 'active'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php echo SVRURL; ?>requisicoes_terminar_entre_datas.php?x=<?php echo base64_encode(2); ?>&&d1=<?php echo base64_encode($d1); ?>&&d2=<?php echo base64_encode($d2); ?>&&ies=<?php echo base64_encode($esc); ?>&&page=<?php echo $i; ?>">
               <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
