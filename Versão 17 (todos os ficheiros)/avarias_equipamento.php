...
if ( empty($_GET["ei"])  || !isset($_GET["ei"])  ||
...
// al e p são opcionais: se vazios, mostra todas as avarias do equipamento
$eq       = (int)    base64_decode($_GET["ei"]);
$al       = !empty($_GET["al"]) ? base64_decode($_GET["al"]) : '';   // string, ano letivo
$per      = !empty($_GET["p"])  ? (int) base64_decode($_GET["p"]) : 0;
$idescola = (int)    base64_decode($_GET["ies"]);
$filtrar_periodo = ($al !== '' && $per > 0);
...
if ( $eq <= 0 || $idescola <= 0 || $idescola > $maxesc ) {
    echo '<script>window.location.href = "' . SVRURL . 'lista";</script>';
    exit;
}

// ── 5. Nome do equipamento, escola e sala ────────────────────────────────────
$stmt_eq = mysqli_prepare($db, "SELECT nomeequi FROM equipamento WHERE id = ?");
mysqli_stmt_bind_param($stmt_eq, 'i', $eq);
mysqli_stmt_execute($stmt_eq);
$rows10 = mysqli_fetch_row(mysqli_stmt_get_result($stmt_eq));
if (!$rows10) {
    echo '<script>window.location.href = "' . SVRURL . 'lista";</script>';
    exit;
}
...
$stmt_sala = mysqli_prepare($db, "SELECT s.nome FROM salas s INNER JOIN equipamento e ON s.id = e.id_sala WHERE e.id = ?");
mysqli_stmt_bind_param($stmt_sala, 'i', $eq);
mysqli_stmt_execute($stmt_sala);
$rows12 = mysqli_fetch_row(mysqli_stmt_get_result($stmt_sala));
if (!$rows12) {
    echo '<script>window.location.href = "' . SVRURL . 'lista";</script>';
    exit;
}
...
if ($filtrar_periodo) {
    $stmt = mysqli_prepare($db,
        "SELECT ar.*, s.nome AS nome_sala
         FROM avarias_reparacoes ar
         INNER JOIN salas s ON ar.id_sala = s.id
         WHERE ar.id_equi = ? AND ar.ano_letivo = ? AND ar.periodo = ? AND ar.id_escola = ?
         ORDER BY ar.dataavaria DESC
         LIMIT ?, ?"
    );
    mysqli_stmt_bind_param($stmt, 'isiiii', $eq, $al, $per, $idescola, $paginationStart, $limit);
} else {
    $stmt = mysqli_prepare($db,
        "SELECT ar.*, s.nome AS nome_sala
         FROM avarias_reparacoes ar
         INNER JOIN salas s ON ar.id_sala = s.id
         WHERE ar.id_equi = ? AND ar.id_escola = ?
         ORDER BY ar.dataavaria DESC
         LIMIT ?, ?"
    );
    mysqli_stmt_bind_param($stmt, 'iiii', $eq, $idescola, $paginationStart, $limit);
}
...
if ($filtrar_periodo) {
    $stmt_total = mysqli_prepare($db,
        "SELECT COUNT(*) FROM avarias_reparacoes
         WHERE id_equi = ? AND ano_letivo = ? AND periodo = ? AND id_escola = ?"
    );
    mysqli_stmt_bind_param($stmt_total, 'isii', $eq, $al, $per, $idescola);
} else {
    $stmt_total = mysqli_prepare($db,
        "SELECT COUNT(*) FROM avarias_reparacoes
         WHERE id_equi = ? AND id_escola = ?"
    );
    mysqli_stmt_bind_param($stmt_total, 'ii', $eq, $idescola);
}
                  ...
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li>
                        <a href="<?php echo SVRURL ?>num_avarias_equipamento.php?x=<?php echo base64_encode(2); ?>&&eq=<?php echo base64_encode($eq); ?>&&ies=<?php echo base64_encode($idescola); ?>"
                           style="color:#4b6cb7;text-decoration:none;">Nº avarias do equipamento</a>
                     </li>
                     <li style="color:#c5cde0;font-size:.9rem;">&#8250;</li>
...
<?php include("msg_bemvindo.php"); ?>
                  ...
                  <!-- Card equipamento + sala + instituição + período -->
                  <div style="display:flex;align-items:center;flex-wrap:wrap;gap:16px;margin:14px 0 10px;padding:12px 16px;background:#f4f6fb;border:1px solid #e3e8f4;border-radius:10px;">
                     ...
                     <div style="display:flex;flex-direction:column;">
                        ...
                        <span style="font-size:.95rem;font-weight:700;color:#182848;">
                            <?php echo $filtrar_periodo ? htmlspecialchars($al, ENT_QUOTES, 'UTF-8') . ' &nbsp;/&nbsp; ' . (int)$per : 'Todos os períodos'; ?>
                        </span>
                     </div>
                  </div>
...
<div class="d-flex flex-row-reverse bd-highlight mb-3">
    <form action="<?php echo SVRURL ?>avarias_equipamento.php?ei=<?php echo base64_encode($eq); ?>&&al=<?php echo base64_encode($al); ?>&&p=<?php echo base64_encode($per); ?>&&ies=<?php echo base64_encode($idescola); ?>" method="post">
...
<div class="gei-table-wrap">
    ...
    <table class="gei-table">
        <thead>
            <tr>
                <th style="width:14%">Ano / Período</th>
                <th style="width:43%">Avaria</th>
                <th style="width:43%">Reparação</th>
            </tr>
        </thead>
        <tbody>
...
<nav aria-label="Paginação">
    <ul class="pagination justify-content-center">
        <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page <= 1) { echo '#'; } else {
                   echo SVRURL . 'avarias_equipamento.php?ei=' . base64_encode($eq)
                      . '&&al='  . base64_encode($al)
                      . '&&p='   . base64_encode($per)
        ...
        <li class="page-item <?php if ($page == $i) echo 'active'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php echo SVRURL; ?>avarias_equipamento.php?ei=<?php echo base64_encode($eq); ?>&&al=<?php echo base64_encode($al); ?>&&p=<?php echo base64_encode($per); ?>&&ies=<?php echo base64_encode($idescola); ?>&&page=<?php echo $i; ?>">
               <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
