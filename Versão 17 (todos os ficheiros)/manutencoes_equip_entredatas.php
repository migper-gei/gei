...
// ── CORRECÇÃO: x=1 é redirect do atualiza_ok — lê parâmetros do GET ──────────
if ($x == 1) {
    $di = base64_decode(urldecode($_GET["dmi"]));
    $df = base64_decode(urldecode($_GET["dmf"]));
    $eq = (int)base64_decode(urldecode($_GET["ei"]));
} elseif ($x == 2 && (empty($_POST['equip']) || !isset($_POST['equip'])
...
$sql2 = "select s.nome from equipamento e, salas s where e.id_sala=s.id and e.id=$eq";
...
<?php if ($num_ns==0 || $num_ne==0 || $num_neq==0): ?>
<script>window.setTimeout(function(){ window.location.href='<?php echo SVRURL ?>manut'; },10);</script>
<?php endif; ?>
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Equipamento entre datas</li>
                  </ol>
               </nav>
               </div>
            </div>
...
<?php include("msg_bemvindo.php"); ?>
               ...
               <div style="display:flex; align-items:center; flex-wrap:wrap; gap:10px; margin:14px 0 10px; padding:10px 16px; background:#f4f6fb; border:1px solid #e3e8f4; border-radius:10px;">
                  ...
                  <span style="display:inline-flex; align-items:center; gap:6px; font-size:1.05rem; font-weight:700; color:#182848;">
                     <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#4b6cb7" stroke-width="2.2" stroke-linecap="round" style="flex-shrink:0;"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
                     <?php echo htmlspecialchars($neq, ENT_QUOTES, 'UTF-8'); ?>
                  </span>
                  <span style="color:#c5cde0;">|</span>
                  <span style="display:inline-flex; align-items:center; gap:6px; font-size:.92rem; font-weight:500; color:#5a6a85;">
                     <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7b88a0" stroke-width="2.2" stroke-linecap="round" style="flex-shrink:0;"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>
...
$sql3 = "
SELECT MIN(m.codigo) as codigo, e.id, e.nomeequi, s.nome,
       m.data_manutencao,
       GROUP_CONCAT(m.descricao ORDER BY m.descricao ASC SEPARATOR '|||') as descricao,
       GROUP_CONCAT(DISTINCT m.pessoa ORDER BY m.pessoa ASC SEPARATOR ', ') as pessoa,
       GROUP_CONCAT(DISTINCT m.observacoes ORDER BY m.observacoes ASC SEPARATOR ' | ') as observacoes
...
GROUP BY e.id, e.nomeequi, s.nome, m.data_manutencao
ORDER BY m.data_manutencao DESC
LIMIT $paginationStart, $limit";
$result = mysqli_query($db, $sql3);

$sql1 = "SELECT count(*) FROM (
    SELECT m.data_manutencao FROM manutencao m, equipamento e, salas s
    WHERE m.id_equi=e.id AND e.id_sala=s.id
    AND m.id_equi=".$eq." AND m.data_manutencao BETWEEN
    STR_TO_DATE('$di','%Y-%m-%d') AND STR_TO_DATE('$df','%Y-%m-%d')
    GROUP BY m.data_manutencao
) as t";
...
.gei-desc-badge{display:inline-flex;align-items:center;padding:4px 10px;border-radius:20px;font-size:.78rem;font-weight:500;color:#2c3e6b;background:#eef2fb;border:1px solid #d0d9f0;white-space:nowrap;margin:2px 3px 2px 0;}
...
<div class="gei-table-wrap">
    <div class="gei-table-toolbar">
        <div class="gei-table-toolbar-left">
            <form action="" method="post" style="margin:0;">
                <input type="hidden" name="datami2" value="<?php echo htmlspecialchars($di, ENT_QUOTES); ?>">
                <input type="hidden" name="datamf2" value="<?php echo htmlspecialchars($df, ENT_QUOTES); ?>">
                <input type="hidden" name="equip"   value="<?php echo (int)$eq; ?>">
    ...
    <table class="gei-table" id="js-sort-table">
        ...
        while ($row2 = mysqli_fetch_array($result)):
            ...
            $sql3b = "SELECT count(*) FROM manutencao WHERE id_equi=".$eq." AND data_manutencao=STR_TO_DATE('$dm','%Y-%m-%d') AND STR_TO_DATE(data_manutencao,'%Y-%m-%d') > STR_TO_DATE('$df2b','%Y-%m-%d')";
            $result3b = mysqli_query($db, $sql3b);
            $rows3b = mysqli_fetch_row($result3b);
            $contama = $rows3b[0];
        ?>
        <tr>
            <td data-label="Data" style="white-space:nowrap;">
                <?php echo htmlspecialchars(date('d/m/Y', strtotime($row2['data_manutencao'])), ENT_QUOTES, 'UTF-8'); ?>
            </td>
            <td data-label="Descrição / Realizada por">
                <div style="display:flex;flex-wrap:wrap;gap:4px;align-items:center;">
                <?php
                $badges = array_filter(array_map('trim', explode('|||', $row2['descricao'])));
                foreach ($badges as $badge):
                    if ($badge !== ''):
                ?>
                    <span class="gei-desc-badge"><?php echo htmlspecialchars($badge, ENT_QUOTES, 'UTF-8'); ?></span>
                <?php
                    endif;
                endforeach;
                ?>
                ...
                <?php if ($row2['pessoa']): ?>
                    <small style="color:#7b88a0;display:block;margin-top:4px;"><?php echo htmlspecialchars($row2['pessoa'], ENT_QUOTES, 'UTF-8'); ?></small>
                <?php endif; ?>
            </td>
            <td data-label="Observações">
                <?php echo htmlspecialchars($row2['observacoes'], ENT_QUOTES, 'UTF-8'); ?>
            </td>
            ...
            <?php if (($_SESSION['tipo']==1 || $_SESSION['tipo']==3) && $contama>0): ?>
                <a class="gei-action-btn gei-btn-edit" title="Atualizar"
                   href="atualiza_manutencao.php?c=<?php echo urlencode(base64_encode($row2['codigo']))?>&da1=<?php echo urlencode(base64_encode($di))?>&da2=<?php echo urlencode(base64_encode($df))?>&eq=<?php echo urlencode(base64_encode($eq))?>&ides=<?php echo urlencode(base64_encode($idescola))?>&origem=equip">
                ...
                <a class="gei-action-btn gei-btn-delete" title="Eliminar"
                   onclick="a(<?php echo htmlspecialchars($row2['codigo'], ENT_QUOTES, 'UTF-8');?>);"
...
<div class="gei-pagination">
    <a class="gei-page-btn <?php if($page<=1) echo 'disabled'; ?>"
       href="<?php echo $page<=1 ? '#' : '?x='.urlencode(base64_encode(2)).'&dmi='.urlencode(base64_encode($di)).'&dmf='.urlencode(base64_encode($df)).'&ei='.urlencode(base64_encode($eq)).'&esm='.urlencode(base64_encode($idescola)).'&page='.$prev; ?>">
    ...
    <a class="gei-page-btn <?php if($page==$i) echo 'active'; ?>"
       href="manutencoes_equip_entredatas.php?x=<?php echo urlencode(base64_encode(2)) ?>&dmi=<?php echo urlencode(base64_encode($di));?>&dmf=<?php echo urlencode(base64_encode($df));?>&ei=<?php echo urlencode(base64_encode($eq));?>&esm=<?php echo urlencode(base64_encode($idescola));?>&page=<?= $i ?>"><?= $i ?></a>
    <?php endfor; ?>
    <a class="gei-page-btn <?php if($page>=$totoalPages) echo 'disabled'; ?>"
       href="<?php echo $page>=$totoalPages ? '#' : '?x='.urlencode(base64_encode(2)).'&dmi='.urlencode(base64_encode($di)).'&dmf='.urlencode(base64_encode($df)).'&ei='.urlencode(base64_encode($eq)).'&esm='.urlencode(base64_encode($idescola)).'&page='.$next; ?>">
