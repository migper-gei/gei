...
if ( $x === 2 && ( empty($_POST['equi']) || !isset($_POST['equi']) ) ) {
    if ( empty($_GET["eq"]) || !isset($_GET["eq"]) ) {
        echo '<script>window.location.href = "' . SVRURL . 'lista";</script>';
        exit;
    }
    $eq = (int) base64_decode($_GET["eq"]);
} elseif ( $x === 0 ) {
    if ( empty($_POST["equi"]) || !isset($_POST["equi"]) ) {
        echo '<script>window.location.href = "' . SVRURL . 'lista";</script>';
        exit;
    }
    $eq = (int) $_POST["equi"];
...
// ── 5. Nome da escola e do equipamento ───────────────────────────────────────
...
mysqli_stmt_bind_param($stmt, 'iii', $eq, $paginationStart, $limit);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

// ── 8. Total para paginação ──────────────────────────────────────────────────
$stmt_total = mysqli_prepare($db,
    "SELECT COUNT(DISTINCT CONCAT(ano_letivo, periodo)) AS total
     FROM avarias_reparacoes
     WHERE id_equi = ?"
);
mysqli_stmt_bind_param($stmt_total, 'i', $eq);
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Nº de avarias do equipamento</li>
                  </ol>
               </nav>
...
<div class="d-flex flex-row-reverse bd-highlight mb-3">
    <form action="" method="post">
        <input type="hidden" name="equi" value="<?php echo (int)$eq; ?>">
...
<div style="padding:30px 16px;text-align:center;color:#7b88a0;font-size:.9rem;background:#f4f6fb;border:1px solid #e3e8f4;border-radius:10px;">
    ...
    Não foram encontradas avarias para este equipamento.
</div>
...
<?php while ($row = mysqli_fetch_array($result)): ?>
            <tr>
                ...
                <td data-label="Nº avarias">
                    <a class="gei-link" title="Ver avarias/reparações"
                       href="<?php echo SVRURL ?>avarias_equipamento.php?ei=<?php echo base64_encode($eq); ?>&&p=<?php echo base64_encode($row['periodo']); ?>&&al=<?php echo base64_encode($row['ano_letivo']); ?>&&ies=<?php echo base64_encode($idescola); ?>">
...
<nav aria-label="Paginação">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page <= 1) { echo '#'; } else {
                   echo SVRURL . 'num_avarias_equipamento.php?x=' . base64_encode(2)
                      . '&&eq='  . base64_encode($eq)
        ...
        <li class="page-item <?php if ($page == $i) echo 'active'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php echo SVRURL; ?>num_avarias_equipamento.php?x=<?php echo base64_encode(2); ?>&&eq=<?php echo base64_encode($eq); ?>&&ies=<?php echo base64_encode($idescola); ?>&&page=<?php echo $i; ?>">
               <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
