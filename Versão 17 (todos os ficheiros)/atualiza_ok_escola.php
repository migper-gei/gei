...
$_id_escola = (int)base64_decode($url[0]);

if ($_id_escola <= 0) { ?>
    <script>window.setTimeout(function(){ window.location.href='<?php echo SVRURL ?>dadosescola'; },10);</script>
<?php
    exit;
}
...
if (empty($_POST['csrf_token']) || empty($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) { ?>
    ...
    .then(function(){ window.location = "<?php echo SVRURL ?>dadosescola"; });
    </script>
<?php
    exit;
}
// Destruir o token usado e gerar imediatamente um novo para a próxima utilização
...
if (empty($_nome)) { ?>
    <script>
    swal({ title: 'Dados inválidos!', text: 'O nome da instituição não pode estar vazio.', icon: 'error' })
    .then(function(){ window.location = "<?php echo SVRURL ?>dadosescola"; });
    </script>
<?php
    exit;
...
// --- UPDATE ---
$stmt = $db->prepare("UPDATE escolas SET nome_escola=? WHERE id=?");
$stmt->bind_param("si", $_nome, $_id_escola);
$stmt->execute();
$stmt->close();
