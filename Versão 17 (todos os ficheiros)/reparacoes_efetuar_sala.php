...
if ($x == 0) {
    $sa       = (int)$_POST["sala"];   // CORRIGIDO: cast (int) — previne SQL Injection
    $idescola = (int)base64_decode($_GET["ies"]);
} elseif ($x == 1) {
    $sa       = (int)base64_decode($_GET["sai"]);
    $idescola = (int)base64_decode($_GET["ies"]);
...
// CORRIGIDO: prepared statements em vez de interpolação directa
$stmt10   = $db->prepare("SELECT nome FROM salas WHERE id=?");
$stmt10->bind_param("i", $sa);
$stmt10->execute();
$rows10   = $stmt10->get_result()->fetch_row();
$stmt10->close();
$ns       = $rows10[0] ?? '';
$num_ns   = $ns !== '' ? 1 : 0;
...
$stmt11->execute();
$rows11   = $stmt11->get_result()->fetch_row();
$stmt11->close();
$ne       = $rows11[0] ?? '';
$num_ne   = ($ne !== '') ? 1 : 0;
            ...
            <div class="row">
               ...
               <nav style="margin-bottom:10px;">
                  <ol style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;list-style:none;padding:0;margin:0;font-size:.8rem;font-weight:600;color:#7b88a0;">
                     ...
                     <li style="color:#1e2a45;">Reparações a efetuar — <?php echo htmlspecialchars($ns, ENT_QUOTES, 'UTF-8'); ?></li>
                  </ol>
               </nav>
...
<?php include("msg_bemvindo.php"); ?>
               ...
               <div style="display:flex; align-items:center; flex-wrap:wrap; gap:16px; margin:14px 0 10px; padding:12px 16px; background:#f4f6fb; border:1px solid #e3e8f4; border-radius:10px;">
                  ...
                  <div style="margin-left:auto; display:flex; gap:8px; align-items:center;">
                     <a title="Reparações a efetuar (todas)"
                        ...
                        style="display:inline-flex;align-items:center;padding:4px 12px;border-radius:6px;font-size:.78rem;font-weight:700;text-decoration:none;
                               <?php echo $op=='t' ? 'background:#253d6e;color:#fff;' : 'background:#e8f0fe;color:#4b6cb7;border:1.5px solid #c7d4f0;'; ?>">
                        Todas
                     </a>
                     <a title="Reparações a efetuar (ano letivo)"
                        ...
                        style="display:inline-flex;align-items:center;padding:4px 12px;border-radius:6px;font-size:.78rem;font-weight:700;text-decoration:none;
                               <?php echo $op=='al' ? 'background:#253d6e;color:#fff;' : 'background:#e8f0fe;color:#4b6cb7;border:1.5px solid #c7d4f0;'; ?>">
                        Ano: <?php echo htmlspecialchars($rows3[0], ENT_QUOTES, 'UTF-8'); ?>
                     </a>
                  </div>
               </div>
...
if ($op == 't') {
    // CORRIGIDO: prepared statements — $sa e $paginationStart/$limit são inteiros seguros
    $stmt_main  = $db->prepare("SELECT * FROM avarias_reparacoes WHERE datareparacao IS NULL AND id_sala=? ORDER BY dataavaria DESC LIMIT ?,?");
    $stmt_main->bind_param("iii", $sa, $paginationStart, $limit);
    $stmt_main->execute();
    $result = $stmt_main->get_result();

    $stmt_cnt   = $db->prepare("SELECT count(*) FROM avarias_reparacoes WHERE datareparacao IS NULL AND id_sala=?");
    $stmt_cnt->bind_param("i", $sa);
    $stmt_cnt->execute();
    $result1 = $stmt_cnt->get_result();
}
if ($op == 'al') {
    $anoLetivo = $rows3[0] ?? '';
    $stmt_main  = $db->prepare("SELECT * FROM avarias_reparacoes WHERE datareparacao IS NULL AND id_sala=? AND ano_letivo=? ORDER BY dataavaria DESC LIMIT ?,?");
    $stmt_main->bind_param("isii", $sa, $anoLetivo, $paginationStart, $limit);
    $stmt_main->execute();
    $result = $stmt_main->get_result();

    $stmt_cnt   = $db->prepare("SELECT count(*) FROM avarias_reparacoes WHERE datareparacao IS NULL AND ano_letivo=? AND id_sala=?");
    $stmt_cnt->bind_param("si", $anoLetivo, $sa);
    $stmt_cnt->execute();
    $result1 = $stmt_cnt->get_result();
}
$rowsCnt     = $result1->fetch_row();
$totallinhas = $rowsCnt[0] ?? 0;
...
<div class="d-flex flex-row-reverse bd-highlight mb-3">
    <form action="<?php echo SVRURL ?>reparacoes_efetuar_sala.php?op=<?php echo $op ?>&&x=<?php echo base64_encode(2) ?>&&sai=<?php echo base64_encode($sa); ?>&&ies=<?php echo base64_encode($idescola); ?>" method="post" name="form1">
...
<div class="gei-table-wrap">
    <div class="gei-table-section-header">
        ...
        Reparações a efetuar — <?php echo htmlspecialchars($ns, ENT_QUOTES, 'UTF-8'); ?>
    </div>
...
?>
            <tr>
                <td data-label="Sala / Equipamento">
                    <span class="gei-label">Equipamento</span>
                    <span style="font-weight:700;"><?php echo htmlspecialchars($noeq, ENT_QUOTES, 'UTF-8'); ?></span>
                    ...
                    <?php if (!empty($row['video'])): ?>
                        <div style="margin-top:8px;">
                            ...
                            <video width="200" height="150" controls style="border-radius:6px;border:1px solid #e3e8f4;">
                                <source src="streamvideo.php?id=<?php echo base64_encode($n); ?>" type="video/mp4">
...
<nav aria-label="Paginação">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page <= 1) { echo '#'; } else {
                   echo '?x='.base64_encode(1).'&&sai='.base64_encode($sa).'&&op='.$op.'&&ies='.base64_encode($idescola).'&&page='.$prev;
               } ?>">&lt;&lt;</a>
        </li>
        ...
        <li class="page-item <?php if ($page == $i) echo 'active'; ?>">
            <a style="color:black;" class="page-link"
               href="reparacoes_efetuar_sala.php?x=<?php echo base64_encode(1); ?>&&sai=<?php echo base64_encode($sa); ?>&&op=<?php echo $op; ?>&&ies=<?php echo base64_encode($idescola); ?>&&page=<?php echo $i; ?>">
               <?php echo $i; ?>
            </a>
        </li>
        <?php endfor; ?>
        ...
        <li class="page-item <?php if ($page >= $totoalPages) echo 'disabled'; ?>">
            <a style="color:black;" class="page-link"
               href="<?php if ($page >= $totoalPages) { echo '#'; } else {
                   echo '?x='.base64_encode(1).'&&sai='.base64_encode($sa).'&&op='.$op.'&&ies='.base64_encode($idescola).'&&page='.$next;
               } ?>">&gt;&gt;</a>
        </li>
