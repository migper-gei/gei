...
?>
        ...
        <div class="d-flex flex-row-reverse bd-highlight mb-3">
            <form action="ver_reparacoes_sala.php?x=<?php echo base64_encode(1) ?>&&sa=<?php echo base64_encode($sa);?>&&di=<?php echo base64_encode($di);?>&&df=<?php echo base64_encode($df);?>&&ies=<?php echo base64_encode ($idescola)?>" method="post">
            ...
            <ul class="pagination justify-content-center">
                <li class="page-item <?php if($page <= 1){ echo 'disabled'; } ?>">
                    <a style="color:black;" class="page-link"
                        href="<?php if($page <= 1){ echo '#'; } else { echo "?x=".base64_encode(1)."&&sa=".base64_encode($sa)."&&di=".base64_encode($di)."&&df=".base64_encode($df)."&&ies=".base64_encode($idescola)."&&page=" . $prev; } ?>"><<</a>
                </li>

                <?php for($i = 1; $i <= $totoalPages; $i++ ): ?>
                <li class="page-item <?php if($page == $i) {echo 'active'; } ?>">
                    <a style="color:black;" class="page-link" href="ver_reparacoes_sala.php?x=<?php echo base64_encode(1);?>&&di=<?php echo base64_encode($di) ?>&&df=<?php echo base64_encode($df) ?>&&sa=<?php echo base64_encode($sa);?>&&ies=<?php echo base64_encode($idescola);?>&&page=<?= $i; ?>"> <?= $i; ?> </a>
                </li>
                <?php endfor; ?>

                <li class="page-item <?php if($page >= $totoalPages) { echo 'disabled'; } ?>">
                    <a style="color:black;" class="page-link"
                        href="<?php if($page >= $totoalPages){ echo '#'; } else {echo "?x=".base64_encode(1)."&&sa=".base64_encode($sa)."&&di=".base64_encode($di)."&&df=".base64_encode($df)."&&ies=".base64_encode($idescola)."&&page=". $next; } ?>">>></a>
                </li>
...
<a href="<?php echo SVRURL ?>num_avarias_entredatas.php?x=<?php echo base64_encode(1)?>&di=<?php echo base64_encode($di) ?>&df=<?php echo base64_encode ($df) ?>&ies=<?php echo base64_encode($idescola);?>">
