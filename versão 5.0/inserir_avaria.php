...
if (!empty($_POST["escola"]))
{
   ...
   $_SESSION['idesc'] =$idescola;
}

$idescola=$_SESSION['idesc'];
...
$sql = "SELECT DISTINCT(nome) as no,id FROM salas where id_escola=$idescola order by nome";
  ...
  <?php 
         ...
         <!--
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST") {
              ...
              <form name="avaria" action="<?php echo SVRURL ?>grava_avaria.php?sa=<?php echo $sa;?>
            
              &&em=<?php echo ($em);?>" method = "post" enctype="multipart/form-data" 
              onSubmit="return enviardados();">

                   <label>Equipamento: (se vazio, sala sem equipamento) </label> 
                   <br>
                   <?php
               echo('<select name="equip" required style="background-color:#CEF6CE">');
                      ...
                      //sala='".$_REQUEST[sala]."'
                       ...
                       $sql = "select id,nomeequi from equipamento where id_sala='$sa' order by nomeequi";
                       $result = mysqli_query($db,$sql); 
